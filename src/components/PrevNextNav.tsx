...
export default function PrevNextNav({ prev, next }: PrevNextNavProps) {
  return (
    <nav className="mt-16 pt-12 border-t border-tn-border flex justify-between gap-6">
      {prev ? (
        <a href={`#/blogs/${prev.slug.split('/').map(encodeURIComponent).join('/')}`} className="group flex-1 min-w-0">
      ...
      {next ? (
        <a href={`#/blogs/${next.slug.split('/').map(encodeURIComponent).join('/')}`} className="group flex-1 min-w-0 text-right">
